...
def convert_to_boxes(z_where, z_pres, z_pres_prob):
    ...
    for b in range(B):
        ...
        conf = z_pres_prob[b][z_pres[b]].unsqueeze(-1)
        # (M, 5)
...
def read_boxes(paths, size = 640):
    boxes_all = []
    for path in paths:
        info: Dict = read_json(os.path.join(path, "info_simple.json"))
        bbox = list(info['bboxes'].values())
        box = []
        for ele in bbox:
            x_min, x_max, y_min, y_max = ele[0], ele[1], ele[2], ele[3]
            box.append([y_min, y_max, x_min, x_max])
        box = np.array(box) / size
